    ...
    <div class="tw-bg-white tw-rounded-2xl tw-p-4 tw-mb-6 tw-shadow-sm">
        <div class="row tw-items-center">
            <div class="col-12 col-md-4 tw-mb-3 tw-mb-md-0">
                <input type="text" id="searchPet" placeholder="Search pets..." class="tw-w-full tw-px-4 tw-py-2 tw-rounded-xl tw-border tw-border-gray-200 focus:tw-border-[#24CFF4] focus:tw-ring-1 focus:tw-ring-[#24CFF4]">
            </div>
            <div class="col-12 col-md-8">
                <div class="tw-flex tw-flex-wrap tw-gap-2">
                    <button class="tw-px-4 tw-py-2 tw-rounded-xl tw-text-sm tw-font-medium tw-transition-all species-filter active" data-species="all">All</button>
                    @foreach($uniqueSpecies as $species)
                    <button class="tw-px-4 tw-py-2 tw-rounded-xl tw-text-sm tw-font-medium tw-transition-all species-filter" data-species="{{ $species }}">{{ Str::plural($species) }}</button>
                    @endforeach
                </div>
            </div>
        </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-md-6 col-lg-4 col-xl-3">
            <div class="tw-bg-white tw-rounded-2xl tw-overflow-hidden tw-shadow-sm tw-transition-all tw-duration-300 hover:tw-shadow-lg hover:-tw-translate-y-1 tw-relative tw-group">
                <button onclick="deletePet({{ $pet->petID }})" class="tw-absolute tw-top-3 tw-left-3 tw-z-20 tw-opacity-0 group-hover:tw-opacity-100 tw-transition-all tw-duration-200 tw-bg-red-500 tw-text-white tw-rounded-full tw-w-8 tw-h-8 tw-flex tw-items-center tw-justify-center hover:tw-bg-red-600 tw-shadow-md">
                    <i class="fas fa-trash-alt tw-text-sm"></i>
                </button>
                <div class="tw-relative">
                    <img src="{{ asset('storage/' . $pet->petImage) }}" alt="{{ $pet->name }}" class="tw-w-full tw-h-48 tw-object-cover">
                ...
                <div class="tw-p-4">
                    ...
                    <div class="tw-flex tw-justify-between tw-items-center">
                        ...
                        <button onclick="editPet({{ $pet->petID }})" class="tw-bg-[#24CFF4] tw-text-white tw-px-4 tw-py-2 tw-rounded-xl tw-transition-all tw-duration-300 hover:tw-shadow-lg hover:tw-opacity-90">
        ...
        <div class="col-12">
            <div class="tw-flex tw-flex-col tw-items-center tw-justify-center tw-bg-white tw-rounded-2xl tw-p-8 tw-shadow-sm">
                ...
                <button type="button" onclick="window.openAddPetModal()" class="tw-bg-[#24CFF4] tw-text-white tw-px-6 tw-py-2 tw-rounded-xl tw-transition-all tw-duration-300 hover:tw-shadow-lg hover:tw-opacity-90">
        ...
        <div id="noResultsMessage" class="col-12 tw-hidden">
            <div class="tw-flex tw-flex-col tw-items-center tw-justify-center tw-bg-white tw-rounded-2xl tw-p-8 tw-shadow-sm">
                <i class="fas fa-search tw-text-5xl tw-text-gray-300 tw-mb-4"></i>
                <p class="tw-text-gray-500 tw-mb-0">No matching records found</p>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function initializePetsPage() {
        ...
        function filterPets() {
            ...
            const visibleCount = Array.from(petCards).filter(card => card.style.display !== 'none').length;
