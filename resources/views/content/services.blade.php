...
<div class="container-fluid tw-min-h-screen tw-overflow-y-auto tw-bg-[#f4fbfd] tw-p-6 font-poppins">
        ...
        <div class="tw-flex tw-flex-col tw-gap-3 md:tw-flex-row md:tw-items-center md:tw-justify-between">
            <div class="tw-flex tw-flex-wrap tw-gap-2" id="category-filters">
                <button
                    type="button"
                    data-category="all"
                    class="category-filter tw-rounded-full tw-bg-[#24CFF4] tw-px-4 tw-py-2 tw-text-sm tw-font-semibold tw-text-white tw-transition-all"
                >
                    All
                </button>
                @foreach ($categories as $category)
                    <button
                        ...
                        class="category-filter tw-rounded-full tw-bg-[#eef7fc] tw-px-4 tw-py-2 tw-text-sm tw-font-semibold tw-text-[#159cbb] tw-transition-all hover:tw-bg-[#dbf5fc]"
                    >
                        {{ $category }}
                    </button>
                @endforeach
            ...
            <div class="tw-relative tw-w-full md:tw-max-w-sm">
                ...
                <input
                    ...
                    class="tw-w-full tw-rounded-xl tw-border tw-border-gray-200 tw-bg-[#f9fcff] tw-py-2 tw-pl-10 tw-pr-4 tw-text-sm tw-text-gray-700 focus:tw-border-[#24CFF4] focus:tw-outline-none"
                >
            </div>
        </div>
    </div>
    ...
    <div class="row tw-mt-2" id="services-grid">
        @forelse ($services as $service)
            ...
            @endphp
            <div
                class="col-12 col-md-6 col-xl-4 tw-mt-4 service-card"
                data-category="{{ $categoryLower }}"
                data-search="{{ strtolower(($service->name ?? '') . ' ' . ($service->description ?? '') . ' ' . ($service->category ?? '')) }}"
            >
                <div class="tw-group tw-h-full tw-overflow-hidden tw-rounded-2xl tw-bg-white tw-shadow-md tw-transition-all tw-duration-300 hover:tw--translate-y-1 hover:tw-shadow-xl">
                    ...
                    <div class="tw-flex tw-h-[calc(100%-13rem)] tw-flex-col tw-p-5">
                        ...
                        <div class="tw-mt-auto">
                            <button
                                type="button"
                                class="service-book-btn tw-w-full tw-rounded-xl tw-bg-[#24CFF4] tw-px-3 tw-py-2 tw-text-sm tw-font-semibold tw-text-white tw-transition-all hover:tw-bg-[#1eb9da]"
                                data-booking-type="{{ $categoryLower === 'boarding' ? 'boarding' : 'appointment' }}"
